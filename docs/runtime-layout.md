...
~/.fleetor/
  ...
  _shell/            everything the running fleet owns
    ...
    state.db           the event log — one SQLite table, `events`, WAL mode
...
- **Nothing under `_shell/` is precious.** The event log is the only state, and it is a diary — a deleted `~/.fleetor` costs history, never correctness of the target repo.

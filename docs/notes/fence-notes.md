...
| `claude` background auto-updater *(added 2026-09-10, D-082)* | updates the operator's own install under `~/.local/share/claude` | **installed every new release into the private HOME** — ~190 MB per version per seat, never run, since a worker's PATH has no private-HOME rung | **fixed by D-082** — fenced Claude Code panes are handed `DISABLE_AUTOUPDATER=1`; measured to write nothing with it set |

...
| **2 — Messaging** | shim, `ask_lead`/`reply`, `dm`, Stop-hook delivery, `await_events` | Scripted 3-agent conversation with a mid-turn delivery, against fake-claude. **Done (GREEN).** The one real-CC confirmation (a live worker using the shim + Stop hook) is deferred to Phase 4 — it needs the multi-worker runner that Phase 4 builds anyway. Wiring is pre-captured in `docs/phase2-spikes.md`. |
| **3 — Quality loop** | Gate runner, auto-bounce with retry cap, peer-review dispatch; **`report()` over MCP** (promoting D-008's transcript-scrape) and the **ownership tools** `whos_working_on`/`claim_file`/`backlog_add` (deferred from Phase 2 per D-015) | A deliberately buggy ticket bounces, gets fixed, passes review — no human input. |
| **4 — Shell** | Tauri app: terminal pane, worker strip, board, ask-lead rail; **the multi-worker fleet runner** (hub + N real workers + lead) | The §7 theme; a full session run end-to-end in the UI. **Includes the deferred Phase 2 real-CC messaging confirmation:** a live Flash worker calls `ask_lead` through the shim and receives mid-turn mail via its Stop hook, answered by the lead — the messaging model proven on real CC, not just fake-claude. Also lands the remaining mail-delivery paths deferred in D-015 — **idle→stdin** and **opportunistic piggyback** — which need the runner driving real turn boundaries. |

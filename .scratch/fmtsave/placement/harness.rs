        ...
        if !posture.is_empty() {
            notices.push((
                ...
                format!(
                    ...
                    posture.iter().map(|(k, v)| format!("{k} {v}")).collect::<Vec<_>>().join(", "),
                ),
            ));
        }
...
///
/// **C58's named second push into [`Host::harnesses`]**, built here because the gate
/// is what needed it: C23 puts Claude Code on the orchestrator row and the workers
/// row beside codex, and a harness whose login the gate cannot speak to sits next to
/// one whose it can, looking half-built. That is story 9's *a supported feature looks
/// unimplemented*, at the one screen where the operator is deciding what to spend.
///
/// **`home` and `path` arrive rather than being read**, which is the whole reason
/// this is a function taking two arguments instead of a function taking none. It
/// lives inside `placement`, `tests/placement_reads_nothing.rs` holds the module to
/// its one rule, and the two facts it needs — the operator's `HOME` and the `PATH`
/// the application was launched with — are already fields on [`Host`]. The values
/// this reads are therefore the same values a spawn resolves against, and a test can
/// hand it a scratch directory.
///
...
/// still works. So a pass here carries the same
/// [`REACHABILITY_NOT_AUTHORIZATION`] sentence codex's does — the two harnesses fail
/// this way for different reasons and the operator is told the same thing.
///
...
/// `placement`, and putting it on [`Host`] to dodge that would put the operator's
/// own key on a value the whole spawn path holds. The refusal names the bound so an
/// operator in that state can read what happened rather than guess.
///
...
/// readings disagreeing absolutely. So the version is read once through the bare
/// name and once through the absolute file the `PATH` resolved to, and whether they
/// agreed is [`HarnessReadiness::readings_agree`]. Zero tokens: `--version` requests
/// no completion.
///
...
pub fn claude_code_diagnose(home: Option<&Path>, path: &str) -> HarnessReadiness {
    ...
    HarnessReadiness {
        ...
        // FLEETOR wrote is a gate check of its own, and Claude Code has no
...
fn claude_code_login(home: Option<&Path>) -> LoginState {
    ...
    let text = match std::fs::read_to_string(&file) {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            return LoginState::NoCredential { summary: no_login_here(&file) }
        }
        Err(e) => {
            return LoginState::Unreadable {
                why: format!("{} could not be read: {e}", file.display()),
            }
        }
    };
    let value: serde_json::Value = match serde_json::from_str(&text) {
        Ok(value) => value,
        // **Not a refusal**, for the reason `LoginState::Unreadable` exists: a
        // working installation whose internal state file this arc cannot parse is a
        // parse error, and treating it as "logged out" would refuse a seat on the
        // strength of one.
        Err(e) => {
            return LoginState::Unreadable {
                why: format!("{} is not readable as JSON: {e}", file.display()),
            }
        }
    };
    match value.get("oauthAccount") {
        ...
        _ => LoginState::NoCredential { summary: no_login_here(&file) },
    }
}

/// The one sentence both no-credential branches say, so they cannot drift apart.
fn no_login_here(file: &Path) -> String {
    format!(
        "{} records no completed `claude` login (an `ANTHROPIC_API_KEY` in your shell is \
         not written there and is not read here)",
        file.display(),
    )
}
...
mod tests {
    ...
    fn the_registry_holds_two_distinct_harnesses_and_names_resolve() {
        ...
        for harness in all {
            ...
            let found =
                by_name(name).unwrap_or_else(|| panic!("{name} is registered and unfindable"));
            assert!(
    ...
    fn seeding_through_the_harness_writes_the_keys_the_spec_names() {
        ...
        let scratch = std::env::temp_dir()
            .join(format!("fleetor-harness-seed-{}", fleetor_core::time::now_ms()));

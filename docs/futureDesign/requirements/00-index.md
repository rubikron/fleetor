...
1. **The system prompt is a shared budget, and it is already over — four times now.** Five packages append prompt text; each doc carries a `brief-cost` line and WP-09 audits the sum. With the `--system-prompt` switch landed (WP-02, D-043), the budget is the *whole* prompt, baseline included. **After WP-07 it is orch 2,668 tokens / worker 2,102** (DeepSeek Flash tokenizer, `examples/system-prompt-spike/count.py`), from WP-02's 1,637 / 1,115 → WP-03's 1,885 / 1,343 → WP-04's 1,910 / 1,343 → WP-05's 2,228 / 1,591 → WP-06's 2,542 / 1,967. Three packages have now overrun by the same multiple for the same reason: WP-03 budgeted ~120 across both files and spent **+248 / +228** (D-045); WP-05 budgeted ~150 and spent **+318 / +248** (D-047); WP-06 budgeted ~150, was warned by Q-2 that the headroom was gone, tightened first and still spent **+314 / +376** (D-050) — its tightening pass recovered only 6% of the addition, which is the evidence that there is no fat left to cut. **WP-07 is the control case for that claim, and it holds** (D-051, Q-4): it budgeted ~60 and spent **+126 / +135** — roughly 2×, against the others' 3–5× — and the difference is that it added *no verb*, so its prose had to teach one name, one word and one authority clause rather than a syntax with rules. The lesson is therefore no longer a guess: **a `brief-cost` line written before the prose is fiction, and a verb costs what its *rules* cost — not what its syntax costs.** Against the 1,485 tokens of CC guidance that stopped being sent, orch is now ~1,180 tokens past parity and a worker ~990 past where it started. WP-09 is a prune, not an audit — and Q-3 argues the *what to cut* call is the operator's, because every remaining candidate is some package's core protocol.

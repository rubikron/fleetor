...
4. **The wire** (`Op` / `OpResult` / `Hello`) — six ops, and the wire tag *is* the CLI verb. `Task` is the only one that never reaches the app: a board that reached a terminal would be a dispatcher.

...
impl Host {
    ...
    /// and each registered harness has one written for it by hand. #34 built
    /// codex's; #35 built Claude Code's (M17 — `~/.claude.json`'s `oauthAccount`,
    /// carrying the same reachability-not-authorization caveat), because the gate
    /// offers both on both rows and a harness with nothing to say about itself sits
    /// beside one that does, looking half-built. A third harness is a third push
    /// into this vector, not a redesign. A trait method would have made it a
    /// fifteenth checkpoint, which is the wrong shape: the fourteen are facts about
    /// a vendor, and this is a fact about a machine.
...
pub enum PaneSpec {
    ...
    Orch {
        ...
        /// default** (M2, M16).
        ///
        /// `None` is the gate's `default (your login)` sentinel and it is the
        /// value that keeps D-052's asymmetry reachable: the operator's own pane,
        /// running their account and whatever model it defaults to, naming
        /// nothing. It stopped being an *invariant* the moment this seat could be
        /// a different vendor, which is what M2 amended; it is still the default.
        ///
        /// **What it is remembering is what was last launched**, not what the pane
        /// is running — changing a model mid-run is the harness's own command and
...
impl PaneSpec {
    ...
    ///
    /// A refinement rather than an argument on the two constructors above, because
    /// the seats that *can* carry a model are two of four and the fleet has one
    /// call site that names one. Every other caller — the conformance suite, the
    /// two judges, every test that places a pane — is asking for the seat and not
    /// for a model, and making them all say `None` would be noise on fifty lines
    /// to serve one.
    ///
...
pub fn place(
    ...
    match spec {
        ...
        // **The fleet's own worker model is the fallback, applied here** — the one
        // place holding both the picked value and the `PaneContext` that names the
        // default, so neither a caller nor a spawn helper has to remember which
        // wins (C9: a worker's provider is FLEETOR's, so an unnamed model is the
        // launch configuration's and never the vendor's).
...
fn place_orch(
    ...
    // orchestrator on the sentinel runs the operator's login and whatever model
    // that account defaults to, which is not a fact this side of the pty. What
    // changed with #35 is that the operator may now say otherwise, and when they do
    // the record says what they said.
    Ok(Placed {
        command,
        notices,
        gauge: None,
...
pub const MISSING_RUSTUP: &str = "no rustup was found — workers will spawn but cannot build Rust. \

...
/// anything relative to the harness that would accept it.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct SeatChoice {
    /// The harness's [`harness::HarnessSpec`] name, which is the same string
    /// `manifest.json` records (M24) and the same one [`harness::by_name`] resolves.
    /// One spelling, three readers.
    pub harness: String,
    /// The model this seat starts with, or `None` for the seat's own default —
    /// the orchestrator's `default (your login)` sentinel (M2), or a worker's
...
impl FleetSeats {
    ...
    ///
    /// **Byte-for-byte the fleet that spawned before the picker existed**, which is
    /// the property that makes shipping it safe: an operator who never opens a
    ...
    fn unpicked(worker_model: &str) -> Self {
        ...
        Self {
            orch: SeatChoice { harness: claude.clone(), model: None },
    ...
    ///
    /// **Two checks and neither of them is about models.** A harness name that no
    /// longer resolves is a fleet that cannot spawn, and a wrong number of workers
    /// is a fleet with a seat nobody chose for. A *model* that no longer exists is
    /// the other kind of problem entirely — it is real, it is #36's, and refusing it
    ...
    fn validated(self) -> Result<Self, String> {
        ...
        if self.workers.len() != expected {
            return Err(
                format!("a fleet has {expected} worker seats, not {}", self.workers.len(),),
            );
        }
    ...
    /// so the caller keeps the arm it already had rather than growing a branch here.
...
/// between a click and a pty, and two callers arriving at once must produce one
/// probe rather than two. A `Mutex` held *across* the probe gives all three: the
/// second caller blocks until the first has finished and then finds the answer
/// already there. That is deliberate rather than a lock held too long.
///
...
impl HarnessGate {
    ...
    fn read(&self) -> (Vec<harness::HarnessReadiness>, Vec<(NoticeLevel, String)>) {
    ...
    fn refresh(&self) -> (Vec<harness::HarnessReadiness>, Vec<(NoticeLevel, String)>) {
        let mut held = match self.0.lock() {
            Ok(held) => held,
            Err(poisoned) => poisoned.into_inner(),
        };
    ...
    /// Forget what was held, without paying for a new reading.
    ///
    ...
    /// the operator who retyped a path is not the operator waiting on a probe.
    fn invalidate(&self) {
...
/// on the spawn path.
fn probe_the_harnesses() -> (Vec<harness::HarnessReadiness>, Vec<(NoticeLevel, String)>) {
...
pub struct HarnessOffer {
    ...
    /// option* rather than hiding the option: a supported feature must never look
...
///
/// One value, so the pickers and the summary line cannot be reading two things. The
...
impl HarnessOffer {
    ...
    /// deliberately not a refusal (C14): a vendor that changed its report format is a
    /// working installation the gate could not parse, and treating that as "logged
...
struct Fleet {
    ...
    /// call would be a choice spread across five invocations of the pty path. It is
    /// written by `fleet_set_seats` before any pane exists and read by
...
pub fn fleet_bootstrap(
    ...
    //
    // **The reading is kept, not just narrated.** #34 threw the `Host` away after
    // emitting its notices; #35's gate renders from it, so it lands on
    // [`HarnessGate`] — one probe answering both the feed and the pickers, which is
    // also what stops the first `fleet_gate` call from paying for a second one.
    ...
    // The fleet nobody has picked yet, which is byte-for-byte the fleet that
    // spawned before the gate had pickers. The worker default comes off the
    // resolved `PaneContext` rather than being spelled a second time here.
...
pub(crate) fn spawn_pane(
    ...
    //
    // **Which harness and model each seat runs is the operator's answer, read
    ...
    // `seats` value, so a fleet the gate described and a fleet that spawns cannot
    // be two different fleets without one of them being read from somewhere else.
    let spec = match seats.spec_for(pane) {
        ...
        None => match pane {
            ...
            // A seat the gate carries a choice for, whose choice would not resolve.
            // `spec_for` refuses rather than substituting, and `fleet_set_seats` is
            // what stops one being stored — this arm is the belt to that braces.
            PaneId::Orch | PaneId::Worker(_) => {
                return Err(format!(
                    "no registered harness answers to `{}`, so this seat cannot be placed",
                    match pane {
                        PaneId::Orch => seats.orch.harness.clone(),
                        _ => "a worker's pick".to_string(),
                    },
                ))
            }
        },
...
pub fn fleet_roster(state: State<'_, FleetState>) -> Result<Vec<PaneEntry>, String> {
    ...
    app.send(AppCommand::Roster { ack }).map_err(|_| {
        "the fleet app is not accepting commands — its window may have closed".to_string()
    })?;
    rx.blocking_recv()
...
fn adopt_target(registry: &PaneRegistry, state: &FleetState, target: &Path) -> Result<(), String> {
...
fn apply_target(state: &FleetState, target: &Path) {
    ...
    if let Ok(mut guard) = state.0.lock() {
        if let Some(fleet) = guard.as_mut() {
            ...
            note(&fleet.store, NoticeLevel::Info, &format!("target set to {}", target.display()));
            if !is_git {
                note(
                    &fleet.store,
                    NoticeLevel::Warn,
...
pub fn fleet_pick_target(
    ...
    let path = picked.into_path().map_err(|e| format!("that folder has no usable path: {e}"))?;
    if !path.is_dir() {
...
pub fn fleet_set_target(
    ...
    let canonical =
        expanded.canonicalize().map_err(|e| format!("resolve {}: {e}", expanded.display()))?;
...
mod tests {
    ...
    fn a_pane_op_crosses_the_real_socket_and_is_answered() {
        ...
        let store: Arc<dyn Store> = Arc::new(SqliteStore::open(&dir.join("state.db")).unwrap());

        let (app_tx, app_rx) = mpsc::unbounded_channel();
        let registry = Arc::new(PaneRegistry::new(Arc::new(|_, _| {}), dir.join("panes.pids")));
        deliver::spawn_delivery(
            &rt,
            registry,
            app_rx,
            store.clone(),
    ...
    fn the_target_is_settable_until_a_pane_exists() {
        ...
        cmd.arg("30");
        registry
            .spawn(PaneId::Orch, cmd, crate::placement::harness::claude_code().spec(), 24, 80)
    ...
    fn a_malformed_config_is_reported_rather_than_ignored() {
        ...
        assert!(
            parse_target(r#"{"target": 7}"#).is_err(),
            "a non-string target is a mistake, not an absence"
        );
    }
    ...
    fn the_composers_target_picks_the_verb_and_never_invents_one() {
        ...
        assert_eq!(
            operator_op("orch", "hi").unwrap(),
            Op::Send { to: PaneId::Orch, text: "hi".into() }
        );
    ...
    fn the_composer_reports_the_fleets_three_words_and_no_others() {
        let accepted = operator_result(OpResult::Delivered {
            msg_id: "msg-1".into(),
            accepted: true,
            detail: None,
        })
        ...
        assert_eq!(
            operator_result(OpResult::Recorded { record_id: "msg-2".into() }).unwrap().outcome,
            "recorded"
        );

        let error =
            operator_result(OpResult::Error { message: "nobody has messaged operator yet".into() })
